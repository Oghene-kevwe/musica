...
export const TopChartsContainer = ({
  ...
  return (
    <article className="remove-scrollbar mb-16  px-4  overflow-auto  scroll-smooth ">
      ...
      <div className="thin-scrollbar overflow-auto md:h-[45vh]  scroll-smooth flex md:flex-col justify-between gap-4">
        {topChart.map((item) => {
          ...
          return (
            <button
              onClick={handleClick}
              key={item.id}
              className=" w-64  md:w-full  h-60 md:h-24 shrink-0 text-light  p-4 rounded-2xl flex justify-between gap-y-2 bg-darkAlt"
            >
